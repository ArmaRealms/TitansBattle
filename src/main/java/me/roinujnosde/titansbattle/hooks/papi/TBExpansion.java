...
public class TBExpansion extends PlaceholderExpansion {
    ...
    static {
        ARENA_IN_USE_PATTERN = Pattern.compile("arena_in_use_(?<arena>[A-zÀ-ú]+)");
        LAST_WINNER_GROUP_PATTERN = Pattern.compile("last_winner_group_(?<game>[A-zÀ-ú]+)");
        LAST_WINNER_KILLER_PATTERN = Pattern.compile("last_(?<type>winner|killer)_(?<game>[A-zÀ-ú]+)");
        PREFIX_PATTERN = Pattern.compile("(?<game>^[A-zÀ-ú]+)_(?<type>winner|killer)_prefix");
